...
class ModelExplainer:
    ...
    def save_round_data(self, round_number, training_loss, validation_loss, evaluation_loss, rmse=None):
        ...
        with open(json_path, 'w') as f:
            json.dump(data, f, indent=4)
        print(f"Dados da round {round_number} salvos em reports/rounds_data.json")
    ...
    def explain(self, round_number, training_loss, validation_loss, evaluation_loss):
        """Gera explicações para o modelo atual."""
        print(f"[Client 1] Gerando explicações...")
        ...
        print(f"[Client 1] Gerando explicações LIME...")
        lime_exp = self.explain_lime()
        
        # Gera explicações SHAP
        print(f"[Client 1] Gerando explicações SHAP...")
